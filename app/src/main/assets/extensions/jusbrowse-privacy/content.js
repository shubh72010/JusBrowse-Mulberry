...
(function () {
    ...
    function getSiteContext(host) {
        return { seed: hash(globalSeed + host), site: host, stable: true };
    }

    const host = window.location.hostname;
    const siteContext = getSiteContext(host);

    // ═══ 1. COOKIE CONSENT BANNER HIDING (non-breaking) ═══
    const cookieBannerSelectors = [
        ...
        "[id*=\"consent\" i].overlay", "[class*=\"consent\" i].overlay"
    ].join(", ");

    const cookieStyle = document.createElement("style");
    cookieStyle.textContent = cookieBannerSelectors + " { display: none !important; visibility: hidden !important; opacity: 0 !important; pointer-events: none !important; }";
    (document.head || document.documentElement).appendChild(cookieStyle);

    // ═══ 2. COSMETIC FILTERING (ad element hiding) ═══

    const cosmeticSelectors = [
        ".ad-banner", ".advertisement",
        "#ad-container", ".ad-container",
        ".adslot", "#adsense", ".adsbygoogle",
        ".googleads", "#doubleclick", ".doubleclick",
        "[data-ad-slot]",
        ".sponsored-content", ".sponsored",
        "#promoted-content", ".promoted",
        "iframe[src*=\"doubleclick\" i]",
        "iframe[src*=\"googlesyndication\" i]",
        "iframe[src*=\"adsense\" i]",
        "iframe[src*=\"adnxs\" i]",
        "iframe[src*=\"amazon-adsystem\" i]",
    ].join(", ");

    const cosmeticStyle = document.createElement("style");
    cosmeticStyle.textContent = cookieBannerSelectors + ", " + cosmeticSelectors +
        " { display: none !important; visibility: hidden !important; opacity: 0 !important; pointer-events: none !important; }";
    (document.head || document.documentElement).appendChild(cosmeticStyle);

    // ═══ 2. MEDIA EXTRACTION (Airlock) ═══
